  ...
  Object.entries(categories).forEach(([categoryKey, category]) => {
    category.faqs.forEach(faq => {
      if (
        ...
        results.push({ ...faq, category: categoryKey, categoryTitle: category.title });
      }
    });
  });
  return results;
...
function CategoryCard({ categoryKey, category, onCategorySelect }) {
  ...
  const className = colorClasses[category.color] ?? 'bg-gray-50 text-gray-700 border-gray-200';

  return (
    <button
      onClick={() => onCategorySelect(categoryKey)}
      aria-label={`View ${category.title} questions`}
...
export default function FAQPage() {
  ...
  // Memoized popular FAQs to avoid recomputation
  const popularFAQs = useMemo(() => {
    return Object.entries(faqCategories)
      .flatMap(([categoryKey, category]) => 
        category.faqs
          .filter(faq => faq.popular)
          .map(faq => ({ ...faq, category: categoryKey, categoryTitle: category.title }))
      );
  }, []);
          ...
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900 flex items-center">
                {React.createElement(faqCategories[selectedCategory].icon, { className: "w-6 h-6 mr-2" })}
            ...
            <div className="space-y-4">
              {faqCategories[selectedCategory].faqs.map((faq) => (
                <FAQItem
