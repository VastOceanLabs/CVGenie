...
const Modal: React.FC<ModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && modalRef.current) {
      ...
      // Handle tab trapping with dynamic element detection
  ...
  // Size classes with proper full-width handling
  const sizeClasses = {
    sm: 'max-w-md mx-4',
    md: 'max-w-lg mx-4',
    lg: 'max-w-2xl mx-4',
    xl: 'max-w-4xl mx-4',
    full: 'max-w-none mx-0 h-full' // True full-width on mobile
  };
    ...
    >
      ...
      <div className={`relative w-full ${sizeClasses[size]} ${containerClassName}`}>
        ...
        >
          ...
          {/* Body */}
          <div className="flex-1 overflow-y-auto px-6 py-4" id={ariaDescribedBy}>
            {children}
          </div>
...
export const ConfirmModal: React.FC<{
  ...
  return (
    ...
    >
      <p id="confirm-modal-description" className="text-gray-600 leading-relaxed">
        {message}
      </p>
    </Modal>
  );
