...
interface ResumeEducationEntry {
  ...
  gpa?: string;
  honors?: string; // Fixed: Added missing property
  coursework?: string;
  location?: string;
  current?: boolean;
  type?: string;
...
// Regex patterns for metrics detection (fixed: reset lastIndex to avoid sticky behavior)
...
export class ATSAnalyzer {
  ...
  public analyze(): ATSAnalysisResult {
    ...
    }, keywordAnalysis, metricsAnalysis);
  ...
  private analyzePersonalInfo(): SectionAnalysis {
    ...
    // Fixed: Improved phone format check
    ...
    return {
      ...
      suggestions,
      status: this.getStatus(finalScore) // Fixed: use clamped score
    };
  }
  ...
  private countActionVerbs(text: string): number {
    ...
    // Fixed: Count actual occurrences, not just presence
