...
import { Plus, Sparkles, BookOpen, ArrowLeft, ArrowRight, GraduationCap, Lightbulb, Target, AlertCircle, CheckCircle, TrendingUp, GripVertical, X, ChevronUp, ChevronDown } from 'lucide-react';
...
export default function EducationSection({ 
  ...
  const [localValidationErrors, setLocalValidationErrors] = useState<Record<number, any>>({});
  ...
  const validateEducation = (education: any, index: number) => {
    const errors: any = {};
    ...
    if (setValidationErrors) {
      setValidationErrors((prev: any) => ({
        ...prev,
        [errorKey]: hasErrors ? errors : undefined
      }));
    }
  ...
  const analyzeEducationATS = (education: any, index: number) => {
  ...
  // Insert suggestion
  const insertSuggestion = (value: string, field: string, index: number) => {
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-green-600">
                {(resumeData?.education || []).filter(e => e.gpa && parseFloat(e.gpa) >= 3.5).length}
              </div>
              <div className="text-xs text-gray-600">High GPA (3.5+)</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-purple-600">
                {(resumeData?.education || []).filter(e => e.honors).length}
              </div>
