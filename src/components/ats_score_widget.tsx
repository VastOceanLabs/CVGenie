...
// Metrics detection regex
const METRICS_REGEX = /\d+%|\$\d+|\d+\+/;
...
function useATSAnalysis(resumeData: ResumeData, template: Template, industryKeywords: string[]): ATSAnalysis {
  const keywordsMemo = useMemo(() => industryKeywords, [industryKeywords.join(',')]);
  ...
  return useMemo(() => {
    ...
    const strengths: ATSAnalysis['strengths'] = [];
  ...
  }, [resumeData, template, keywordsMemo]);
}
