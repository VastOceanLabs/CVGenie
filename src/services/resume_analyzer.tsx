...
const analyzeResume = (resumeData, industry = 'Software Engineer') => {
  ...
  analysis.sectionScores.summary = summaryScore;

  // Experience
  const experienceScore = analyzeExperienceSection(resumeData.experience, rules);
  analysis.sectionScores.experience = experienceScore;

  // Skills
  const skillsScore = analyzeSkillsSection(resumeData.skills, rules);
  analysis.sectionScores.skills = skillsScore;

  // Education
  const educationScore = analyzeEducationSection(resumeData.education, rules);
  analysis.sectionScores.education = educationScore;

  // Calculate weighted overall score with safety cap
  analysis.overallScore = Math.min(100, Math.round(
    summaryScore.score * rules.sectionWeights.summary +
    experienceScore.score * rules.sectionWeights.experience +
    skillsScore.score * rules.sectionWeights.skills +
    educationScore.score * rules.sectionWeights.education
  ));
...
const analyzeSummarySection = (personalInfo, rules) => {
  let score = 0;
  const issues = [];
  ...
  // Keyword presence
  const keywordCount = rules.requiredKeywords.filter(keyword => 
...
const analyzeExperienceSection = (experience, rules) => {
...
const analyzeSkillsSection = (skills, rules) => {
  ...
  const techSkillMatches = skills.filter(skill => 
    rules.technicalSkills.some(techSkill => 
...
const analyzeEducationSection = (education, rules) => {
...
const generateRecommendations = (analysis, resumeData, rules) => {
  ...
  Object.entries(analysis.sectionScores).forEach(([section, data]) => {
    if (data.score < 70) {
      recommendations.push({
        ...
        description: data.issues[0] || `${section} section needs improvement`,
...
const generateStrengths = (analysis, resumeData) => {
  const strengths = [];
...
const generateWarnings = (analysis, resumeData) => {
  const warnings = [];
...
const generateCriticalIssues = (analysis, resumeData) => {
  const issues = [];
