...
const validationRules: {
  [K in Section]?: SectionRules<ResumeData[K]>
} = {
  personalInfo: {
...
export function useResumeData(initialData?: Partial<ResumeData>): UseResumeDataReturn {
  ...
  const totalSections = useMemo(() => {
    ...
    const optionalSections = ['education', 'certifications', 'projects'];
  ...
  const validateField = useCallback((section: Section, field: string, value: any): string | null => {
    ...
    const fieldRule = (sectionRules as Record<string, FieldRule>)[field];
